...
var drawBatjeA = function() {
    ...
    fill(255, 255, 255);
    stroke(255, 255, 255);
    rect(batjeA.breedte, batjeA.hoogte, 10*resize, batjeA.size);
};
...
var drawBatjeB = function() {
    ...
    fill(255, 255, 255);
    stroke(255, 255, 255);
    rect(batjeB.breedte, batjeB.hoogte, 10*resize, batjeB.size);
};
...
var drawPowerUps = function (){
    ...
    if (PowerUpNr === 7){
        puTwoBall();
        if(Math.sqrt(Math.pow(ball.Xpos - powerUp.X,2)*resize + Math.pow(ball.Ypos - powerUp.Y,2))*resize <= 27*resize){
            ball2.Xpos = ball.Xpos*resize;
            ball2.Ypos = ball.Ypos*resize;
            richting.X2 = richting.X*resize;
            richting.Y2 = richting.Y*resize;
            richting.Y = -richting.Y*resize;
            PowerUpNr = -7;
        }
    }
    if (PowerUpNr === -7){
        if (richting.Y <= 0 || richting.Y >= 0){
            ...
            if (counter === 60){
                ball2.Xpos = 500*resize;
                ball2.Ypos = 500*resize;
                PowerUpNr = 0;
                counter = 0;
            }
        }
...
batjesUpdate = function() {
    ...
    if (batjeB.hoogte > 390*resize - batjeB.size) {
        batjeB.hoogte = 390*resize - batjeB.size;
    }
    if (batjeB.hoogte < 10*resize) {
        batjeB.hoogte = 10*resize;
    }
    if (batjeA.hoogte > 390*resize - batjeA.size) {
        batjeA.hoogte = 390*resize - batjeA.size;
    }
    if (batjeA.hoogte < 10*resize) {
        batjeA.hoogte = 10*resize;
    }
};
...
var drawRules = function() {
    ...
    fill(0, 0, 0);
    textSize(14*resize);
    text("Uitleg", 263*resize, 325*resize);
};
